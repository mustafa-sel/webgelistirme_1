...
const NormalDifficulty = ({ navigation }) => {
    ...
    const getQuiz = async () => {
        ...
        setOptions(shuffleOptionsArray(data.results[0]))
        setIsLoading(false)
    };
    ...
    const shuffleOptionsArray=(_question)=>{
    ...
    const handlSelectedOption = (_option) => {
        if (_option === questions[ques].correct_answers) {
            ...
            setQues(ques + 1)
            setOptions(shuffleOptionsArray(questions[ques + 1]))
        } if (ques === 9) {
            handleShowResult()
        }
    }
...
export default NormalDifficulty;
